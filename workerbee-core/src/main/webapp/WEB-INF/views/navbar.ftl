<page-header>
    <div class="navbar">
        <div class="navbar-inner">
            <div class="container">
                <button type="button" class="btn btn-navbar" data-toggle="collapse" data-target="#nav-collapse-01">
                </button>
                <a href="#fakelink" class="brand">Pattaya</a>
                <div class="nav-collapse collapse" id="nav-collapse-01">
                    <ul class="nav">
                        <li class="dropdown">
                            <a href="#" class="dropdown-toggle" data-toggle="dropdown">호스트 <i class="icon-caret-down"></i></a>
                            <ul class="dropdown-menu">
                                <li><a href="#fakelink">모든 호스트</a></li>
                            </ul>
                        </li>
                        <li class="dropdown">
                            <a href="#" class="dropdown-toggle" data-toggle="dropdown">서비스 <i class="icon-caret-down"></i></a>
                            <ul class="dropdown-menu">
                                <li><a href="#fakelink">모든 서비스</a></li>
                                <li class="divider"></li>
                                <li><a href="#fakelink">하둡</a></li>
                                <li><a href="#fakelink">하이브</a></li>
                                <li><a href="#fakelink">우지</a></li>
                            </ul>
                        </li>
                    </ul> <!-- /nav -->

                    <ul class="nav pull-right">
                        <li class="dropdown">
                            <a href="#" class="dropdown-toggle" data-toggle="dropdown">관리 <i class="icon-caret-down"></i></a>
                            <ul class="dropdown-menu">
                                <li><a href="#fakelink">설정</a></li>
                                <li><a href="#fakelink">사용자</a></li>
                                <li class="divider"></li>
                                <li><a href="#fakelink">Separated link</a></li>
                            </ul>
                        </li>
                    </ul> <!-- /nav -->

                    <ul class="nav pull-right">
                        <li class="dropdown">
                            <a href="#" class="dropdown-toggle" data-toggle="dropdown">
                                <i class="icon-gear icon-large hidden-navbar-collapse"></i>
                                <span class="visible-navbar-collapse">설정  <i class="icon-caret-down"></i></span>
                            </a>
                            <ul class="dropdown-menu">
                                <li><a href="#fakelink">메시지</a></li>
                                <li><a href="#fakelink">정보 수정</a></li>
                                <li class="divider"></li>
                                <li><a href="${rootPath}/j_spring_security_logout">로그 아웃</a></li>
                            </ul>
                        </li>
                    </ul> <!-- /nav -->


                    <form class="navbar-search form-search pull-right" action="">
                        <div class="input-append">
                            <input type="text" class="search-query span2" placeholder="Search">
                            <button type="submit" class="btn btn-large">
                                <i class="fui-search"></i>
                            </button>
                        </div>
                    </form>
                </div><!--/.nav-collapse -->
            </div>
        </div>
    </div> <!-- /navbar -->
</page-header>